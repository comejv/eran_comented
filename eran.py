...
class ERAN:
    def __init__(self, model, session=None, is_onnx=False):
        ...
        print('This network has ' +
              str(self.optimizer.get_neuron_count()) + ' neurons.')
    ...
    def analyze_box(self, specLB, specUB, domain, timeout_lp, timeout_milp, use_default_heuristic,
        ...
        # Come step 2:
        # Initialize the analyzer according to chosen domain
        if domain == 'deepzono' or domain == 'refinezono':
            execute_list, output_info = self.optimizer.get_deepzono(
                nn, specLB, specUB)
            # Come
            # the Analyser class is defined l.115 in the file analyzer.py
            ...
            execute_list, output_info = self.optimizer.get_deeppoly(
                nn, specLB, specUB, lexpr_weights, lexpr_cst, lexpr_dim, uexpr_weights, uexpr_cst, uexpr_dim, expr_size, spatial_constraints)
            analyzer = Analyzer(execute_list, nn, domain, timeout_lp, timeout_milp, output_constraints,
                                use_default_heuristic, label, prop, testing, K=K, s=s,
                                timeout_final_lp=timeout_final_lp, timeout_final_milp=timeout_final_milp,
                                use_milp=use_milp, complete=complete,
                                ...
                                approx_k=approx_k)
        # Come step 3:
        # Run the analyzer with function analyse defined l.201 in the file analyzer.py
    ...
    def analyze_zonotope(self, zonotope, domain, timeout_lp, timeout_milp, use_default_heuristic, output_constraints=None, testing=False, prop=-1):
        ...
        """
        assert domain in [
            'deepzono', 'refinezono'], "domain isn't valid, must be 'deepzono' or 'refinezono'"
        nn = layers()
        ...
        if domain == 'deepzono' or domain == 'refinezono':
            ...
            analyzer = Analyzer(execute_list, nn, domain, timeout_lp, timeout_milp,
                                output_constraints, use_default_heuristic, testing, prop=prop)
        elif domain == 'deeppoly' or domain == 'refinepoly':
            assert 0
